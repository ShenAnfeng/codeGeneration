...
<head>
    ...
    <script type="text/javascript">
    	...
    	$(document).ready(function() {
    		$('#resultTab').datagrid({
    			url : "${classNameLower}Controller/${classNameLower}List",
    			idField : 'id',
    			singleSelect : true,
    			multiSort : true,
    			striped : true,
		...
		function _view(rowIndex, rowData) {
			...
			$(dialogDiv).dialog({
			    ...
			    resizable : true,
			    href : "${classNameLower}Controller/${classNameLower}View?id=" + rowData.id,
			    modal : true,
			    onClose : function () {
		...
		function _edit() {
			...
			$(dialogDiv).dialog({
			    ...
			    height : document.body.scrollHeight-100,
			    resizable : true,
			    href : "${classNameLower}Controller/${classNameLower}Edit?id=" + selRec.id,
			    modal : true,
			    onClose : function () {
			    	$('#${classNameLower}Div').dialog('destroy');
			    }
		...
		function _add() {
			...
			$(dialogDiv).dialog({
			    ...
			    height : document.body.scrollHeight-100,
			    resizable : true,
			    href : "${classNameLower}Controller/${classNameLower}Add",
			    modal : true,
			    onClose : function () {
			    	$('#${classNameLower}Div').dialog('destroy');
			    }
		...
		function _del() {
			...
			$.messager.confirm('确认对话框', '请确认是否提交，提交后数据将删除。', function(r) {
				if (r){
					$.ajax({
						url : "${classNameLower}Controller/${classNameLower}Del",
						dataType : "json",
						type : "POST",
						//async : false,
						data : {
